      ...
      <div className="aspect-square">
        ...
        {isHovered && (
          <div
            ...
            onClick={() =>
              callback(nft.media[0]?.thumbnail || nft.media[0]?.gateway, {
                chain,
...
export default function UserProfile({ details, initialData }) {
  ...
  const [profileImage, setProfileImage] = useState(
    details?.profile?.pfp || '/default-avatar.png'
  );
  const [coverImage, setCoverImage] = useState(
    details?.profile?.cover || '/default-cover.jpg'
  );
  ...
  // Fetch points dynamically
  useEffect(() => {
    const fetchPoints = async () => {
      if (!details?.did) {
        console.error('DID is undefined');
        return;
      }

      setLoadingPoints(true);
      try {
        console.log('Fetching profile data for DID:', details.did);
        const { data: profile, status } = await orbis.getProfile(details.did);
        console.log('Profile data:', profile);

        if (profile) {
          // Ensure `details.profile` exists
          if (!details.profile) {
            details.profile = {};
          }

          // Ensure `profile.details.metadata` exists
          if (!profile.details?.metadata) {
            profile.details.metadata = {};
          }

          // Initialize `points` if it doesn't exist
          if (profile.details.metadata.points === undefined) {
            profile.details.metadata.points = 0;
          }

          // Update the points in the `details` object
          details.profile.points = profile.details.metadata.points;
          console.log('Updated points:', details.profile.points);
          console.log('Updated details object:', details);
        } else {
  ...
  const awardPoints = async (points) => {
    ...
    try {
      ...
      if (updateRes.status === 200) {
        console.log(`Awarded ${points} points! Total points: ${newPoints}`);
        details.profile.points = newPoints; // Update local state
      } else {
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-3">
              <div className="bg-gray-50 rounded-lg p-6">
                <div className="flex items-center">
                  ...
                  <span className="ml-2 text-2xl font-bold">
                    {donationStats.totalReceived} ETH
                  </span>
                </div>
              ...
              <div className="bg-gray-50 rounded-lg p-6">
                <div className="flex items-center">
                  {loadingPoints ? (
                    <LoadingCircle className="w-6 h-6 text-gray-500" />
                  ) : (
                    <span className="text-2xl font-bold">
                      {details?.profile?.points || 0}
                    </span>
                  )}
                </div>
